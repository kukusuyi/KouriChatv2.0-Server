...
def debuggerLogger(name: Optional[str] = None, level: int = logging.DEBUG):
    """调试日志记录器"""
    logger = LoggerConfig(".").setup_logger(name, level)
    return logger

def infoLogger(name: Optional[str] = None, level: int = logging.INFO):
    """信息日志记录器"""
    logger = LoggerConfig(".").setup_logger(name, level)
    return logger
